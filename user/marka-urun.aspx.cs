...
{
    public partial class marka_urun : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                Session["sMarkaId"] = dr["MarkaId"].ToString();

                SqlCommand cm = new SqlCommand("spMarkaUrunListesiCount", cn._Baglan());
                cm.CommandType = CommandType.StoredProcedure;
                cm.Parameters.AddWithValue("@MarkaId", Session["sMarkaId"].ToString());
                Session["sMarkaPath"] = dr["MarkaPath"].ToString();

                ddMarka.DataBind();
                ddMarka.SelectedValue = Session["sMarkaPath"].ToString();
        ...
        {
            DataRow dr = cn.DataRowGetir("Select * From Markalar Where MarkaPath = '" + ddMarka.SelectedValue + "'");
            int _MarkaId = Convert.ToInt32(dr["MarkaId"].ToString());
            Session["sMarkaId"] = Convert.ToInt32(_MarkaId);

            Response.Redirect("/marka-kategori/?marka=" + _MarkaId + "&kategori=" + ddKategori.SelectedValue);
        }
