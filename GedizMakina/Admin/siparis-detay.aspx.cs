...
namespace GedizMakina.Admin
{
    public partial class siparis_detay : System.Web.UI.Page
    {
        ...
        void SiparisGetir()
        {
            ...
            DataRow drFaturaAdres = cn.DataRowGetir("Select * From CariAdresler Where AdresId = " + dt.Rows[0]["FaturaAdresId"].ToString() + "");
            lblFaturaAdresi.InnerText = drFaturaAdres["Adres"].ToString();
            lblFaturaSehir.InnerText = drFaturaAdres["Sehir"].ToString();
            lblFaturaIlce.InnerText = drFaturaAdres["Ilce"].ToString();

            DataRow drSevkiyatAdres = cn.DataRowGetir("Select * From CariAdresler Where AdresId = " + dt.Rows[0]["SevkiyatAdresId"].ToString() + "");
            lblSevkiyatAdresi.InnerText = drSevkiyatAdres["Adres"].ToString();
            lblSevkiyatSehir.InnerText = drSevkiyatAdres["Sehir"].ToString();
            lblSevkiyatIlce.InnerText = drSevkiyatAdres["Ilce"].ToString();
